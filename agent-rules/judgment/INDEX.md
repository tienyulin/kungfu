# JUDGMENT INDEX — 通用判斷增量與路由（與 agent-rules Constitution 並用的常駐層）

> 本檔是 Judgment 制度的**常駐入口**：只含 agent-rules Constitution 沒有覆蓋的判斷法則＋路由表。
> Constitution 管執行紀律（驗證、重現、diff、三振…），本檔管「要不要做、做多少、信不信、怎麼想」。
> 完整制度在 @JUDGMENT@/ ——**不要全部讀**，照下方路由按需載入，一次一檔。
> `@JUDGMENT@` 由注入器換成實際路徑。
> **KERNEL.md 在本模式下不要讀**：它是獨立使用（無 agent-rules Constitution）時的全集入口，
> 內容 = Constitution 已有的一半 ＋ 本檔七條——整合模式下讀它等於雙重載入。

## 七條增量法則（Constitution 之外的判斷）
...
1. 任何時刻 context = Constitution＋本檔＋**最多 2 個** Judgment 檔。要載第三檔，先放掉一檔。
2. 每個 domains 檔自包含——載對的那一檔就夠，不需要交叉讀。
3. 與 Constitution 重疊的主題（三振、逐字引用、完成定義、被質疑先查證）以 Constitution 為準，Judgment 檔是其展開。
