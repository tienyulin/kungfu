...
1. **正確性 (correctness)** - 邏輯錯、off-by-one、運算子錯、條件反了、漏 await、race condition、用錯變數
2. **資料損失/安全** - trust boundary 沒驗輸入、injection、secret 寫進程式碼、破壞性操作沒防護、漏 auth 檢查
3. **失敗路徑 (failure paths)** - I/O 失敗會怎樣？輸入空/null/0 筆/10^6 筆？被吞掉的 exception？
4. **Breaking change** - API/schema 改動炸到呼叫方——去查誰在呼叫
