...
`python <本 skill 目錄>/scripts/lint_frontmatter.py README.md docs/` 必須 pass
（`<本 skill 目錄>` = 這個 SKILL.md 所在資料夾的絕對路徑；Claude Code 可直接用
`${CLAUDE_SKILL_DIR}`，其他 agent 沒有這變數就填絕對路徑）。
...
FastAPI：`python <本 skill 目錄>/scripts/gen_openapi.py --app app.main:app` → 產
