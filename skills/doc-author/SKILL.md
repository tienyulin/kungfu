...
查無的已改成「（待補：<問題>）」。**glossary 禁用寫法掃描**：glossary 表列名詞
「文件裡怎麼寫」欄有標「不寫 X」的，逐一 grep 兩份文件（例
`grep -nE '預演|乾跑|乾執行|模擬模式|端點介面' README.md docs/ARCHITECTURE.md`），
命中就改成 glossary 的寫法。**行為描述另抽 3 條**（優先抽：預設值、身分/權限、
