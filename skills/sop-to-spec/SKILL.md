...
- 輸出：`specs/<sop-slug>-api.spec.md`（`specs/` = SOP 所在 repo 根下，不存在就建）；
  `<sop-slug>` = SOP 檔名去副檔名。**一份 SOP = 一份 spec**，不管 SOP 裡有幾隻 API
