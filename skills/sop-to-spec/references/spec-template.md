...
- 前置條件編號 `PC-<n>`（供 §6 與 AC 引用），response 不回逐條結果——dry_run 失敗回
  **第一個沒過的** PC 對應的 `error.status`。
...
### §7 審計
欄位 schema（含 operator＝X-Operator、operation_id）、寫入時機（每個 mutation request
恰好一筆；**401/422 一律不留**）、result 封閉枚舉 `success` / `dry_run` /
