...
本 repo 在 Claude Code 端是一個 **plugin marketplace**（其他 agent 走各自的
committed adapter，見〈跨 agent 同步〉），任何 git 主機都能託管
...
加 `--constitution` 再把 Constitution＋Safety 接進非 Claude agent 的 dotfile（opt-in、冪等，
逐 agent 細節見〈支援矩陣〉）：
...
（應看到 bundle + agent-rules + 外部 mirror plugins）；其他 agent 重開 session 即生效。
...
- skills、Constitution hook、Safety 全部照裝，完全不用 `claude` CLI（腳本偵測不到 `claude`
  會自動跳過 plugin 段）。
...
| plugin | 做什麼 |
|---|---|
| [`agent-rules`](agent-rules/rules/CONSTITUTION.md) | Constitution SessionStart hook ＋ Safety PreToolUse hook（見下章）。做成 hook 不做 skill：**skill 不保證被載入，hook 保證**。 |
...
| **Situation**（按需讀） | [DECISIONS](agent-rules/rules/DECISIONS.md)（問 vs 做查表）、[SAFETY](agent-rules/rules/SAFETY.md)（護欄協定）、[ANTIPATTERNS](agent-rules/rules/ANTIPATTERNS.md)（15 種失敗氣味） | Constitution 說何時讀；hook 一併注入**絕對路徑**，agent 要用時自己開（不常駐、不脹 context） |
| **Safety**（機械強制） | [`guard.py`](agent-rules/hooks/guard.py) 在 PreToolUse 層攔 `rm -rf`、force-push、`git reset --hard`、`DROP`、無 WHERE 的 DELETE、`sudo`… ——**模型自不自覺都過不了關**；擋下的理由文字引導 agent 走 SAFETY §1 協定（亮指令→使用者同意→使用者跑） | hook，同下表 |

### 支援矩陣（腳本自動偵測，只碰你機器上有的 agent）

| agent | skills | Constitution（常駐注入） | Safety |
|---|---|---|---|
...
- **自家 skill 靠上表的機制到手**：kungfu 自己就是個 superpowers-style adapter repo，
  `skills-sync` 用各 agent 的原生機制把 kungfu 裝進去（Gemini/Codex）或 skill-drop
  （OpenCode/Cline）——完整說明見下方〈跨 agent skills 同步細節〉。Constitution/guard 一律另走 hook。
...
卡住走三振升級（LOOP ESCALATED）而不是無限重試；Safety 在圈內照常生效。
...
adapter＋一個 `skills/using-kungfu` bootstrap meta-skill。`skills-sync` 偵測到 agent，就用它的原生
機制把 kungfu 自己裝進去，或退回 skill-drop：

| agent | 裝法 | bootstrap（using-kungfu）載入？ |
|---|---|---|
| Gemini | `gemini extensions link <repo>`（`gemini-extension.json`；link＝活連結，重跑自動反映） | ✅（`GEMINI.md` `@import`） |
| Codex | `codex plugin marketplace add <repo>` ＋ `plugin add kungfu@kungfu-dev`（`.codex-plugin/`＋`.agents/plugins/marketplace.json`） | ✗（Codex 靠 skill discovery，無 session hook） |
| OpenCode | skill-drop `skills/*/` → `~/.agents/skills`（原生讀；`opencode plugin` 要 npm package，本機 clone 裝不了） | ✅（`.opencode/plugins/kungfu.js` 注入第一則訊息） |
| Cline | skill-drop `skills/*/` → `~/.cline/skills`（無原生安裝機制） | ✗ |

內容跟著 marketplace clone 的 auto-update／重跑走。Constitution/guard 一律另走 hook（見〈支援矩陣〉），
跟這層無關。**外部 mirror（`external-skills.json`）**也是同一套 adapter-或-drop 邏輯，見上方
〈外部開源 skills〉。
...
- **這裡改的是 Claude 通道**（`marketplace.json`）。要讓外部 skill 也進
  Gemini／Codex／OpenCode／Cline，另往 [`external-skills.json`](external-skills.json) 加同一筆
  （`name`／`url`／`ref`）——`skills-sync.sh` 讀它、用各 agent 的原生機制裝。兩個清單獨立維護。

### 改 agent-rules

- Constitution/Situation/playbook：直接改 md，merge 即生效（hook 現讀檔，全隊零動作）。
