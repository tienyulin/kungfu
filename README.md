...
**② 跑腳本**——它偵測你有的 agent 就裝哪些（Claude：bundle plugin＋auto-update；Gemini／
Codex／OpenCode：skill-drop 進共用 `~/.agents/skills`，三家原生共讀；Cline：skill-drop 進 `~/.cline/skills`）。
加 `--constitution` 再把 Constitution＋Guard＋using-kungfu nudge 接進非 Claude agent 的 dotfile（opt-in、冪等，
逐 agent 細節見〈支援矩陣〉）：
...
| [`using-kungfu`](skills/using-kungfu/SKILL.md) | bootstrap meta-skill：動手前先挑對的 kungfu skill。作為 nudge，其內文由 `--constitution` 隨 Constitution 一起在 session 開頭注入各 agent（跟 Constitution 互補，不重述工作紀律）。 |
...
[`external-skills.json`](external-skills.json)（name/url/ref）。每個 repo 先 clone 一份，
再依 agent 分發：

| agent | 怎麼裝 |
|---|---|
| Claude Code | repo 自帶 `.claude-plugin/marketplace.json` → `claude plugin marketplace add <url>` ＋ install；否則 drop `skills/*/` 進 `~/.claude/skills`（Claude 原生探索） |
| Gemini / Codex / OpenCode | 一律 drop `skills/*/` 進共用 `~/.agents/skills`（三家都原生讀） |
| Cline | drop `skills/*/` 進 `~/.cline/skills` |

**為什麼非 Claude agent 全走 skill-drop、不裝 Gemini extension／Codex plugin**：
`~/.agents/skills` 是 Gemini／Codex／OpenCode **共讀**的目錄，一份餵三家。若再幫某 repo 裝
extension／plugin，那家就多一份拷貝（`~/.gemini/extensions/<name>/skills`…）疊在共用 drop 上
→ 每次啟動 warn「skill overridden/conflict」——這正是自家 skill [#68](https://github.com/tienyulin/kungfu/pull/68)
解掉的 double-load，外部 skill 同樣處理（並 migrate-off 之前殘留的 extension／plugin）。
OpenCode 另有 bun installer 只吃 `git+<scheme>://` 的限制（scp 式 ssh url 會
`NpmInstallFailedError`），skill-drop 也一併避開。原生 adapter 唯一提供的 bootstrap nudge，
改由 `--constitution` 統一注入 [`using-kungfu`](skills/using-kungfu/SKILL.md)（見〈心法＋戒律〉）。
...
| **Skill nudge**（常駐，`--constitution`） | [`using-kungfu`](skills/using-kungfu/SKILL.md) 內文：動手前先掃可用 kungfu skill、命中就用（skill-usage 紀律，補 Constitution 的工作紀律） | 隨 Constitution 同一支 hook 注入各 agent，餵滿觸發提醒——取代非 Claude agent 原本靠外部 adapter bootstrap 才有的 nudge |
