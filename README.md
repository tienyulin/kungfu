...
| OpenCode | drop 進 `~/.agents/skills`（OpenCode 原生讀，adapter/純 skill 皆同） | drop 進 `~/.agents/skills`（OpenCode 原生讀） |
...
OpenCode 是例外：它原生掃 `~/.agents/skills/*/SKILL.md`，所以一律 skill-drop、不用
`opencode plugin`——後者在 opencode 1.17.15 的 bun installer 只吃 `git+<scheme>://`，
scp 式 ssh url（`git@host:owner/repo.git`）會 `NpmInstallFailedError`，且 skill-drop 對
adapter／純 skill 效果相同（`.opencode` plugin 只注入 bootstrap nudge，非 skill 探索）。
