...
加 `--constitution` 再把憲法＋SAFETY guard 接進非 Claude agent 的 dotfile（opt-in、冪等，
逐 agent 細節見〈支援矩陣〉）：
...
- skills、憲法 hook、SAFETY guard 全部照裝，完全不用 `claude` CLI（腳本偵測不到 `claude`
  會自動跳過 plugin 段）。
...
| [`using-kungfu`](skills/using-kungfu/SKILL.md) | bootstrap meta-skill：動手前先挑對的 kungfu skill。Gemini/OpenCode 會在 session 開頭自動載（跟憲法互補，不重述工作紀律）。 |
...
- **自家 skill 靠上表的機制到手**：kungfu 自己就是個 superpowers-style adapter repo，
  ...
  （OpenCode/Cline）——完整說明見下方〈跨 agent skills 同步細節〉。憲法/guard 一律另走 hook。
...
`~/kungfu/skills-sync.sh`（加 `--constitution` 含跨 agent 憲法/guard；`agents` 模式只碰
...
內容跟著 marketplace clone 的 auto-update／重跑走。憲法/guard 一律另走 hook（見〈支援矩陣〉），
