...
| **Overfitting to tests**（測試過了、需求沒達） | DONE MEANS 只剩測試項在打勾、「非測試觀察」那項一直沒驗；或你改了測試讓它過（憲法 Law 7 直接禁止） | 先跑「非測試觀察」項（實際啟動/呼叫看行為）；動過測試檔 → revert，回頭修程式碼 |
| **Context drift**（拿舊假設做新輪） | 這輪引用的錯誤訊息不是**這輪** gate 貼出來的；或引用的檔案內容是你改過之前的版本 | 重跑 gate、重讀檔案，用新輸出重寫假設（憲法 Law 4／ANTIPATTERNS #13） |
