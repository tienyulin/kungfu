...
wire_constitution() {
  ...
  echo "→ Constitution ＋ using-kungfu nudge（--constitution）：用各家 hook 在 session 開頭注入（讀 marketplace 檔，內容自動跟新）"
  ...
  # using-kungfu bootstrap nudge — the always-on "before acting, reach for a matching
  # kungfu skill" reminder (skill-usage discipline; complements the constitution's
  # work discipline). Injected alongside the constitution so gemini/codex/opencode —
  # which now get skills via the shared ~/.agents/skills drop with no bootstrap of
  # their own — still get the trigger-priming that superpowers' extension provided.
  # We inject the skill BODY (YAML frontmatter stripped).
  local nudge_file="$home/.agents/agent-rules-using-kungfu.md"
  awk 'BEGIN{n=0}/^---$/{n++;next}n>=2{print}' "$SCRIPT_DIR/skills/using-kungfu/SKILL.md" 2>/dev/null > "$nudge_file"
  ...
  local nudge_ref nudge_tilde
  nudge_ref="$(homeref "$nudge_file" sh)"
  nudge_tilde="$(homeref "$nudge_file" tilde)"
...
SHEOF
  ...
  if [ "$codex" = 1 ]; then
    ...
    merge_json_hook "$home/.codex/hooks.json" "SessionStart" \
      "{\"matcher\":\"startup|resume\",\"hooks\":[{\"type\":\"command\",\"command\":\"bash \\\"$refresh_ref\\\"; cat \\\"$const_ref\\\" \\\"$paths_ref\\\" \\\"$nudge_ref\\\"\",\"statusMessage\":\"agent-rules constitution\",\"timeout\":30}]}"
...
paths = [os.path.expanduser(p) for p in ["$const_tilde", "$paths_tilde", "$nudge_tilde"]]
...
PYEOF
  ...
  if [ "$opencode" = 1 ]; then
    opencode_add_instructions "$home/.config/opencode/opencode.json" "$const_src" "$paths_file" "$nudge_file"
...
python3 - "$const_ref" "$paths_ref" "$nudge_ref" <<'PY'
import json, os, sys
...
SHEOF
        ...
        [ -n "$cline_dir" ] && rm -f "$cline_dir/agent-rules-constitution.md" "$cline_dir/agent-rules-situational-paths.md" "$cline_dir/agent-rules-using-kungfu.md"
      ...
      cp "$nudge_file" "$cline_dir/agent-rules-using-kungfu.md"
...
sync_external_skills() {  # <home> <cline_present> <cline_skills>
  ...
  local name url ref clone sel cmp cplug
  while IFS=$'\t' read -r name url ref; do
    ...
    # Gemini, Codex and OpenCode ALL read ~/.agents/skills/*/SKILL.md natively (the
    # same shared dir our own skills use in wire_own_skills), so ONE skill-drop feeds
    # all three. We deliberately do NOT install a Gemini extension or a Codex plugin
    # for the repo: that hands the agent a SECOND copy (~/.gemini/extensions/<name>/
    ...
    # double-load against the shared drop above (the bug this replaced).
    if command -v gemini >/dev/null 2>&1 && [ -d "$home/.gemini/extensions/$name" ]; then
      ( set +o pipefail; yes | gemini extensions uninstall "$name" >/dev/null 2>&1 ) || true
      echo "    Gemini   − 移除舊 $name extension（改走 ~/.agents/skills，免重複載入）"
    fi
    if command -v codex >/dev/null 2>&1; then
      sel="$(codex plugin list 2>/dev/null | grep -oE "${name}@[A-Za-z0-9_.-]+" | head -1 || true)"
      if [ -n "$sel" ]; then
        codex plugin remove "$sel" </dev/null >/dev/null 2>&1 || true
        codex plugin marketplace remove "${sel#*@}" </dev/null >/dev/null 2>&1 || true
        echo "    Codex    − 移除舊 ${sel} plugin（改走 ~/.agents/skills）"
      fi
    fi
