...
SHEOF
    ...
    if [ "$codex" = 1 ]; then
      ...
      merge_json_hook "$home/.codex/hooks.json" "SessionStart" \
        "{\"matcher\":\"startup|resume\",\"hooks\":[{\"type\":\"command\",\"command\":\"bash '$refresh'; cat '$const_src' '$paths_file'\",\"statusMessage\":\"agent-rules constitution\",\"timeout\":30}]}"
...
subprocess.Popen(["bash", "$refresh"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
